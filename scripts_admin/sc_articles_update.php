...
if (isset($_POST["title"]) && ($_POST["title"] != "") && (isset($_POST["desc_s"]) && ($_POST["desc_s"] != ""))) {
    ...
    $id_articles = $_SESSION["id"];

    // quando não ha upload de ficheiros mantem a varaivel de sessão 
    if($_FILES["images_s_path"]["name"] == ""){
      $images_s_path = $_SESSION["images_s_path"];
    } else {
      $images_s_path = $_FILES["images_s_path"]["name"]; 
    }

    if($_FILES["images_lg_path"]["name"] == ""){
      $images_lg_path = $_SESSION["images_lg_path"];
    } else {
      $images_lg_path = $_FILES["images_lg_path"]["name"]; 
    }

    unset($_SESSION['id']);
    unset($_SESSION["images_s_path"]);
    unset($_SESSION["images_lg_path"]);
    ...
    $query = "UPDATE mp_article
              SET title = ?, images_s_path = ?, images_s_desc = ?, images_lg_path = ?, images_lg_desc = ?, desc_s = ?, desc_lg = ?
              WHERE id_articles = ?";

    if (mysqli_stmt_prepare($stmt, $query)) { 
        
        mysqli_stmt_bind_param($stmt, "sssssssi", $title, $images_s_path, $images_s_desc, $images_lg_path, $images_lg_desc, $desc_s, $desc_lg, $id_articles);  /* Bind paramenters */
